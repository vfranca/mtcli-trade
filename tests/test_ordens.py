...
def mock_mt5(mocker):
    ...
    mt5_mock.orders_get.return_value = [MagicMock(ticket=123, symbol="WINV25", type=2, volume_current=1.0, price_open=150000)]
...
def test_formatar_ordem(mock_mt5):
    ...
    ordem = MagicMock(symbol="WINV25", type=2, volume_current=1.0, price_open=150000, ticket=123)
...
def mock_ordens_model(mocker):
    mocker.patch("mtcli_trade.models.ordens_model.buscar_ordens", return_value=[MagicMock(ticket=1)])
    mocker.patch("mtcli_trade.models.ordens_model.formatar_ordem", return_value={"ticket": 1})
    mocker.patch("mtcli_trade.models.ordens_model.cancelar_ordens", return_value=[{"retcode": 10009}])
    return mocker
...
def test_cancelar_ordens_pendentes(mock_ordens_model):
...
@pytest.mark.parametrize(
    "ordens, esperado",
    [
        ([{"symbol": "WINV25", "preco": 150000, "volume": 1, "ticket": 123}], "WINV25"),
        ([], "nenhuma ordem"),
    ],
)
...
def test_cli_ordens_lista(runner):
    with patch("mtcli_trade.controllers.ordens_controller.obter_ordens_pendentes") as mock_exec:
        mock_exec.return_value = [{"symbol": "WINV25", "preco": 150000, "ticket": 123}]
...
def test_cli_ordens_cancelar(runner):
    with patch("mtcli_trade.controllers.ordens_controller.cancelar_ordens_pendentes") as mock_cancel:
...
def test_cli_ordens_filtrar_symbol(runner):
    with patch("mtcli_trade.controllers.ordens_controller.obter_ordens_pendentes") as mock_exec:
        mock_exec.return_value = [{"symbol": "WDOX25", "preco": 5100, "ticket": 999}]
        start = time.perf_counter()
