...
def test_executar_compra_falha_preparacao(mocker):
    ...
    mocker.patch("mtcli_trade.controllers.compra_controller.preparar_ordem_compra", return_value=(None, False))
    resultado = executar_compra("WINV25", 1.0, 150, 300, False, None)
...
def test_executar_compra_excecao_inesperada(mocker):
    mocker.patch("mtcli_trade.controllers.compra_controller.verificar_risco", return_value=False)
    mocker.patch("mtcli_trade.controllers.compra_controller.preparar_ordem_compra", side_effect=RuntimeError("Erro MT5"))
    resultado = executar_compra("WINV25", 1.0, 150, 300, False, None)
    assert resultado["status"] == "erro"
